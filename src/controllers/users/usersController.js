...
exports.getProfileOverview = async (req, res) => {
  ...
  try {
    const userRes = await pool.query(`
      SELECT u.points, u.username, u.profile_picture, rl.role_name, rl.discount_percentage, rl.required_points
      FROM users u
    ...
    const cartCount = await pool.query(`SELECT COUNT(*) FROM shopping_cart WHERE user_id = $1`, [userId]);
    const wishlistCount = await pool.query(`SELECT COUNT(*) FROM wishlist WHERE user_id = $1`, [userId]);
...
exports.setBirthday = async (req, res) => {
  ...
  try {
    const result = await pool.query(`SELECT birthday FROM users WHERE user_id = $1`, [userId]);
    if (result.rows[0].birthday) {
      return res.status(400).json({ message: 'Birthday is already set.' });
    }

    await pool.query(`UPDATE users SET birthday = $1 WHERE user_id = $2`, [birthday, userId]);
...
exports.updateUserPreferences = async (req, res) => {
  const userId = req.user.userId;
  const { theme_mode, language, preferred_currency, email_notifications } = req.body;
